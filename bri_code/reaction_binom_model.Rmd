...
# Step 1: Summarize the data - Count nudibranchs that moved towards or away for each combination of trial_type, temp_grp, and direction_binom
nudi_data_summary <- nudi_data %>%
  group_by(trial_type, temp_grp, direction_binom) %>%
  summarise(count = n(), .groups = 'drop')  # Count nudibranchs in each group
print(nudi_data_summary)

# Step 2: Calculate total number of nudibranchs in each trial_type and temp_grp
nudi_data_totals <- nudi_data %>%
  group_by(trial_type, temp_grp) %>%
  summarise(total = n(), .groups = 'drop')  # Calculate total number of nudibranchs

# Step 3: Join the summarized data with the total counts to calculate proportions
...
# Step 4: Ensure that 'direction_binom' is a factor with appropriate levels (0 = Away, 1 = Towards)
nudi_data_prop$direction_binom <- factor(nudi_data_prop$direction_binom, levels = c(0, 1))

# Step 5: Handle missing data for "towards" direction (if no nudibranchs moved in that direction)
nudi_data_prop_complete <- nudi_data_prop %>%
  complete(trial_type, temp_grp, direction_binom, fill = list(count = 0))  # Add missing rows

# Step 6: Set any NA values in proportion to 0 (if there are no nudibranchs for a particular direction)
nudi_data_prop_complete$proportion[is.na(nudi_data_prop_complete$proportion)] <- 0

# Step 7: Generate the summary table with proportions
summary_table <- nudi_data_prop_complete %>%
  pivot_wider(names_from = direction_binom, values_from = proportion, names_prefix = "Direction_") %>%
  mutate(across(starts_with("Direction_"), ~round(.x, 3)))  # Round proportions for clarity

# View the summary table to check counts and proportions
print(summary_table)
...
plot_rxn_probs <- ggplot(nudi_data_prop_complete, aes(x = factor(temp_grp), y = proportion, fill = factor(direction_binom))) +
  ...
  scale_y_continuous(expand = c(0, 0)) +  # Fixed y-axis scale across both graphs
...
# Display the plot
print(plot_rxn_probs)

```


```{r}
ggsave(plot = plot_rxn_probs, filename = here("bri_code", "plot_rxn_probs.png"), bg = "white")
...
y_max <- max(nudi_data_prop_overall$proportion, na.rm = TRUE)
...
ggplot(nudi_data_prop_overall, aes(x = factor(temp_grp), y = proportion, fill = factor(direction_binom))) +
  geom_bar(stat = "identity", position = "dodge", width = 0.7) +  # Bar chart with dodged bars for direction and adjusted bar width
  labs(
    title = "",
    x = "Treatment Group (°C)",
    y = "Proportion of Nudibranchs",
  ...
  theme(
    ...
    axis.title.x = element_text(margin = margin(t = 10)),  # Add space between x-axis labels and numbers
    axis.title.y = element_text(margin = margin(r = 10)),  # Add space between y-axis labels and numbers
    strip.text = element_text(size = 12, face = "bold", margin = margin(b = 15))  # Add space between facet labels and graph
  )
